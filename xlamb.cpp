...
void xlamb(double &x, double &xpl, int &n, int m, double q, double qsqfm1, int tin){
	double t0, dt, d2t, d3t, tdiff, w, xm, xmold, xtest;
	double tmin, tdiffm, d2t2, tdiff0;
	bool solnflag = false;
	bool termflag = false;
	...
	if (m == 0) { // single rev starter from t (@ x = 0)
		...
		if (thr2 < 0.5){
			...
			printf("glambert does not have code for this eventuallity, thr2 == 0.5 '('xlamb.cpp, line 50')'\n");
		}
		...
		for(int i = 0; i < 12, ++i){
			...
			xmold = xm;
			xm = xm - dt * dt2 / (d2t * d2t - dt * d3t / 2.0);
			xtest = fabs(xmold / mx - 1.0);
